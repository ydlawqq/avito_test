...
Вход — датасет из experiments/xgb_rerank/build_dataset.py:
...
Запуск из корня проекта:
    python experiments/xgb_rerank/train_xgb_reranker.py                    # полный
    python experiments/xgb_rerank/train_xgb_reranker.py --iterations 50    # smoke
...
def build_xy(df: pd.DataFrame, categories: dict[str, list[int]] | None = None):
    X_num = df[NUMERIC_FEATURES].to_numpy(dtype=np.float32)
...
def main() -> None:
    ...
    X_tr, y_tr, g_tr = build_xy(train_df, categories)
    X_ev, y_ev, g_ev = build_xy(eval_df, categories)
    ...
    if holdout_df is not None:
        ...
        X_h, _, g_h = build_xy(holdout_df, categories)
